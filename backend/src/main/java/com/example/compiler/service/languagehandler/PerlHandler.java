...
public class PerlHandler extends AbstractLanguageHandler {
    @Override
    public String executeCode(String code, String stdin) throws IOException, InterruptedException {
        File tempDir = Files.createTempDirectory("perl_exec_" + UUID.randomUUID()).toFile();
        try {
            File file = new File(tempDir, "script.pl");
            writeCodeToFile(file, code);

            ProcessBuilder processBuilder = new ProcessBuilder("perl", file.getAbsolutePath())
                    .directory(tempDir);
